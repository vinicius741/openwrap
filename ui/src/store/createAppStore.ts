...
type AppStore = {
  ...
  configureSelectedProfileGeneratedPassword: (input: {
    username: string
    pin: string
    totpSecret: string
  }) => Promise<void>
  clearSelectedProfileGeneratedPassword: () => Promise<void>
...
export const createAppStore = () => {
  return create<AppStore>((set, get) => {
    const mergeProfileDetail = (updatedProfile: ProfileDetail) => {
      set((state) => ({
        profiles: state.profiles.map((profile) =>
          profile.id === updatedProfile.profile.id
            ? {
                ...profile,
                name: updatedProfile.profile.name,
                remote_summary: updatedProfile.profile.remote_summary,
                has_saved_credentials: updatedProfile.profile.has_saved_credentials,
                validation_status: updatedProfile.profile.validation_status,
              }
            : profile,
        ),
        selectedProfile:
          state.selectedProfile?.profile.id === updatedProfile.profile.id
            ? updatedProfile
    ...
    return {
      ...
      updateSelectedProfileDnsPolicy: async (dnsPolicy) => {
        ...
        try {
          ...
          mergeProfileDetail(updatedProfile)
          set({ error: null })
        } catch (error) {
          set({ error: normalizeCommandError(error) })
        }
      ...
      configureSelectedProfileGeneratedPassword: async ({ username, pin, totpSecret }) => {
        const selectedProfile = get().selectedProfile
        if (!selectedProfile) return
        try {
          const updatedProfile = await configureGeneratedPassword(
      ...
      clearSelectedProfileGeneratedPassword: async () => {
        const selectedProfile = get().selectedProfile
        if (!selectedProfile) return
        try {
          const updatedProfile = await clearGeneratedPassword(selectedProfile.profile.id)
          mergeProfileDetail(updatedProfile)
          set({ error: null })
        } catch (error) {
          set({ error: normalizeCommandError(error) })
