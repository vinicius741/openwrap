...
pub(crate) fn write_launch_config(
...
pub(crate) fn cleanup_runtime_artifacts(active_session: &super::state::ActiveSession) {
    cleanup_auth_file(active_session);
    cleanup_runtime_bridge(active_session);
}

pub(crate) fn cleanup_auth_file(active_session: &super::state::ActiveSession) {
    if let Some(auth_file) = &active_session.auth_file {
        let _ = fs::remove_file(auth_file);
    }
}

fn cleanup_runtime_bridge(active_session: &super::state::ActiveSession) {
...
pub(crate) fn cleanup_runtime_dir(runtime_dir: &Path) {
