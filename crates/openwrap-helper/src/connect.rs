...
use crate::request::{read_json_request, validate_request};
use crate::system::kill_process;

pub async fn run_connect() -> i32 {
    let request = match read_json_request::<ConnectRequest>() {
        Ok(request) => request,
        Err(error) => {
            eprintln!("{error}");
            return 64;
    ...
    let status = tokio::select! {
        ...
        _ = recv_signal(&mut sigterm, &mut sigint) => {
            let _ = kill_process(openvpn_pid.map(|p| p as i32).unwrap_or(0));
            child.wait().await.ok()
        }
    };
